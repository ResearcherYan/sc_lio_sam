...
private:
  ...
  // @yan
  pcl::PointCloud<PointType>::Ptr denseCloud;
...
public:
  ImageProjection() : deskewFlag(0)
  {
    ...
    pubDenseCloud = nh.advertise<sensor_msgs::PointCloud2>("sc_lio_sam/deskew/cloud_dense", 1);
  ...
  void allocateMemory()
  {
    ...
    denseCloud.reset(new pcl::PointCloud<PointType>());

    fullCloud->points.resize(N_SCAN * Horizon_SCAN);
    denseCloud->points.resize(N_SCAN * Horizon_SCAN);
  ...
  void resetParameters()
  {
    ...
    std::fill(denseCloud->points.begin(), denseCloud->points.end(), nanPoint);
  }
  ...
  void imuDeskewInfo()
  {
    ...
    for (int i = 0; i < (int)imuQueue.size(); ++i)
    {
      ...
      // 在 [timeScanCur - 0.01, timeScanCur] 内不断对 cloudInfo.imu***Init 覆盖赋值
      ...
      // 这里相当于用的是 [timeScanCur, timeScanEnd + 0.01] 范围内的 imu 数据对角速度进行积分，用于后面去畸变
  ...
  void publishClouds()
  {
    ...
    cloudInfo.cloud_dense = publishCloud(&pubDenseCloud, denseCloud, cloudHeader.stamp, lidarFrame);
